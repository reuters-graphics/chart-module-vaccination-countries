...
var VaccinationLollipop = /*#__PURE__*/function (_BaseChartComponent) {
  _inherits(VaccinationLollipop, _BaseChartComponent);

  var _super = _createSuper(VaccinationLollipop);

  function VaccinationLollipop() {
    var _this;

    _classCallCheck(this, VaccinationLollipop);
  ...
  _createClass(VaccinationLollipop, [{
    key: "draw",
    ...
    value: function draw() {
      ...
      var useMilestone, milestoneIndex;
      ...
      var yScale = d3.scaleBand().rangeRound([0, height]).domain(data.map(function (d) {
        return d.country;
      })).padding(props.padding); // const transition = d3.transition()
      //   .duration(500);

      var navOnClick = function navOnClick(country) {
        if (props.countryLinks) {
          var countrySlug = client.getCountrySlug(country); // calling this function will navigate to a region page

          props.countryLinks(countrySlug);
        }
      };

      this.selection().appendSelect('p.axis-label').text(props.topText).style('padding-left', props.margin.left - 2 + 'px');
      ...
      var xAxisTop = plot.appendSelect('g.axis.xTOP');
      xAxisTop.attr('transform', 'translate(0,0)').call(d3.axisTop(xScale).ticks(4).tickFormat(function (d) {
        return d * 100;
      }));
      ...
      if (milestones[milestoneIndex - 1] && node.getBoundingClientRect().width > props.annotationHideBreakpoint) {
        ...
        a1text.text(Mustache__default['default'].render(props.text.milestone, {
          number: useMilestone * 100
        }));
      }
  ...
  return VaccinationLollipop;
}(ChartComponent);

module.exports = VaccinationLollipop;
